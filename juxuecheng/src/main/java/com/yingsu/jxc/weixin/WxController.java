...
public class WxController {
    ...
    public static R authorize() {
        String appid = "wxa2fbe7ca7c970259";
        String url ="http://www.juxuecheng.com";
        String result = "";
    ...
    public ModelAndView home(HttpServletRequest request, HttpSession session) throws Exception {
        ...
        log.info("------------------获取用户code,,,用户code="+code);
        // 获取网页授权access_token
        Oauth2Token oauth2Token = getOauth2AccessToken(WeixinConfig.APPID, WeixinConfig.SECRET, code);
        // 用户标识
        String openId = oauth2Token.getOpenId();
        session.setAttribute(Constant.USER_OPENID,openId);
        log.info("------------------获取openId,,,用户openId="+openId);
    ...
    public ModelAndView weixinLogin(HttpServletRequest request, HttpSession session) throws Exception {
        ModelAndView mv = new ModelAndView();
        // 用户同意授权后，能获取到code
        Map<String, String[]> params = request.getParameterMap();//针对get获取get参数
        String[] codes = params.get("code");//拿到code的值
        ...
        if (!"authdeny".equals(code)) {
            ...
            //具体业务start
            TWeixinLogin weixinLogin = new TWeixinLogin();
            BeanUtils.copyProperties(snsUserInfo,weixinLogin);
            // 查找用户是否注册过
            TWeixinLogin weixinLogin1 = wxService.getWeixinUser(weixinLogin.getOpenId());
            log.info("------------------查找用户是否注册过");
            if (weixinLogin1 == null) {
                wxService.addWeixinUser(weixinLogin);
                log.info("------------------注册用户");
            }
            // 信息放入session
            session.setAttribute(Constant.USER_INFO,weixinLogin);
            mv.setViewName("home");
        } else {
